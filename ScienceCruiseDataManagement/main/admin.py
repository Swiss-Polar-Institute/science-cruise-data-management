...
class OutcomeReportFilter(admin.SimpleListFilter):
    title = "Outcome"
    ...
    def __init__(self, request, params, model, model_admin):
        if self.parameter_name in params:
            ...
            print("number 1",self._value)
        elif self.parameter_name in request.session:
            self._value = request.session[self.parameter_name]
            print("number 2", self._value)
        else:
            self._value = None

        super(OutcomeReportFilter, self).__init__(request, params, model, model_admin)
    ...
    def lookups(self, request, model_admin):
        ...
        # sampling_methods = main.models.ParentDevice.objects.all().order_by('name')
        #
        # filter_lookup = []
...
class StationReportFilter(admin.SimpleListFilter):
    title = "Station"
    ...
    def __init__(self, request, params, model, model_admin):
        if self.parameter_name in params:
            request.session[self.parameter_name] = params[self.parameter_name]
            self._value = params[self.parameter_name]
        elif self.parameter_name in request.session:
            self._value = request.session[self.parameter_name]
        else:
            self._value = None

        super(StationReportFilter, self).__init__(request, params, model, model_admin)
...
class SamplingMethodFilter(admin.SimpleListFilter):
    ...
    def __init__(self, request, params, model, model_admin):
        ...
        super(SamplingMethodFilter, self).__init__(request, params, model, model_admin)
...
#
# class EventReportFilter(admin.ListFilter):
#     title = "Sampling method"
#     parameter_name = 'sampling_method'
#     template = 'admin/options_filter.html'
#
#     def __init__(self, request, params, model, model_admin):
#         super(EventReportFilter, self).__init__(request, params, model, model_admin)
